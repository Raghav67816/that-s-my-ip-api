...
from utils.db import user_collection, query_ip_address, UserDataQuery
...
# Ip lookup
@app.post("/ip_lookup")
async def ip_look_up(ip_details: Ip_Lookup):
    validate_ip = check_ip(ip_details.ip)
    if validate_ip == 1:  # IP Address is valid
        validate_req = validate_request(ip_details.name, ip_details.api_key)
        if validate_req == 1:
            ip_records = query_ip_address(ip_details.ip)
            return ip_records
